// server.js
// Entry point. Plain Node `http` server — no Express.
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (method === "POST" && pathname === "/api/auth/forgot") {
      return await authRoutes.forgotPassword(req, res, await readJsonBody(req));
    }
    ...
    const countryDocsMatch = pathname.match(/^\/api\/countries\/([^/]+)\/documents$/);
    if (countryDocsMatch && method === "GET") {
      return countryRoutes.listDocuments(req, res, decodeURIComponent(countryDocsMatch[1]));
    }
    const adminDocsMatch = pathname.match(/^\/api\/admin\/countries\/([^/]+)\/documents$/);
    if (adminDocsMatch && method === "POST") {
      return countryRoutes.addDocument(req, res, await readJsonBody(req), decodeURIComponent(adminDocsMatch[1]));
    }
    const adminDocIdMatch = pathname.match(/^\/api\/admin\/countries\/([^/]+)\/documents\/([^/]+)$/);
    if (adminDocIdMatch && method === "PUT") {
      return countryRoutes.updateDocument(req, res, await readJsonBody(req), decodeURIComponent(adminDocIdMatch[1]), decodeURIComponent(adminDocIdMatch[2]));
    }
    if (adminDocIdMatch && method === "DELETE") {
      return countryRoutes.removeDocument(req, res, decodeURIComponent(adminDocIdMatch[1]), decodeURIComponent(adminDocIdMatch[2]));
    }
